...
class CenterOfMass:
    ...
    def __init__(self, filename, ptype):
        ...
        # NOTE: If the snapshot mass is in 1e10 Msun units, multiply self.m by 1e10
        # e.g. self.m = self.data['m'][self.index] * 1e10
        self.m = self.data['m'][self.index] * 1e10          
    ...
    def COM_P(self, delta=0.1, volDec=2.0):
        ...
        # 4) Find the maximum 3D distance from this COM, then reduce it by volDec
        r_max = np.max(r_new) / volDec
        ...
        while (change > delta):
            ...
            # Decrease r_max by factor volDec again
            r_max /= volDec
